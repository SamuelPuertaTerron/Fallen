...
namespace FEngine
{
    ...
    void Filesystem::CreateFile(const Path& path)
    {
        ...
        Logger::Log(Logger::ELogType::Debug, "Created file at path " + path.string());
    }
    ...
    void Filesystem::WriteText(const Path& path, const std::string& text, EFileWriteMode mode)
    {
	    switch (mode)
	    {
